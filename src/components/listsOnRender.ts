...
export async function listsOnRender() {
  const data = await getGenres();
  const genres = data["genres"].map((e: Genre) => [e["name"], e["id"]]);
  console.log(genres);
  ...
  const data1 = await getMovieWithGenre(randomGenre1[1], 1);
  const data2 = await getMovieWithGenre(randomGenre2[1], 1);
